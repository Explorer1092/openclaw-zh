...
x-i18n:
  ...
  provider: pi
  source_hash: 3d7239ca8185138b55467a1dfa83a443645dc3d15dca10970ca52467ab4002b5
...
- **心跳**是计划性的**主会话轮次**——不创建任务记录。
- **定时任务（主会话）**是计划性的**系统事件注入主会话**——创建带 `silent` 通知策略的任务记录。
- **定时任务（隔离式）**是计划性的**后台运行**——创建在 `openclaw tasks` 中跟踪的任务记录。

所有定时任务执行（主会话和隔离式）均会创建[任务记录](/automation/tasks)。心跳轮次不会。主会话定时任务默认使用 `silent` 通知策略，因此不会产生通知。
...
- **无任务记录**：心跳轮次保留在主会话历史中（参见[后台任务](/automation/tasks)）。
...
周期性整点调度会自动以确定性的每任务偏移量在 0-5 分钟窗口内分散。
...
- **精确定时**：支持带时区的 5 字段或 6 字段（秒级）cron 表达式。
- **内置负载分散**：周期性整点调度默认错开最多 5 分钟。
- **每任务控制**：用 `--stagger <duration>` 覆盖错峰，或用 `--exact` 强制精确定时。
- **会话隔离**：在 `cron:<jobId>` 中运行，不会污染主会话历史。
- **模型覆盖**：可按任务使用更便宜或更强大的模型。
- **投递控制**：隔离任务默认以 `announce` 投递摘要；按需选择 `none`。
- **即时投递**：announce 模式直接发布，无需等待心跳。
- **无需智能体上下文**：即使主会话空闲或已压缩，也能运行。
- **一次性支持**：`--at` 用于精确的未来时间戳。
- **任务跟踪**：隔离任务创建[后台任务](/automation/tasks)记录，可在 `openclaw tasks` 和 `openclaw tasks audit` 中查看。
...
- 该工具是**可选插件**；建议通过 `tools.alsoAllow: ["lobster"]` 附加启用（推荐）。
- Lobster 要求 `lobster` CLI 在 PATH 中可用。
...
| -------------------------------- | ------------------------ | ---------------------- | --------------------------------------------------- |
| 会话                             | 主会话                   | 主会话（通过系统事件） | `cron:<jobId>` 或自定义会话                         |
| 历史                             | 共享                     | 共享                   | 每次运行全新（隔离式）/ 持久化（自定义）            |
| 上下文                           | 完整                     | 完整                   | 无（隔离式）/ 累积（自定义）                        |
| 模型                             | 主会话模型               | 主会话模型             | 可覆盖                                              |
| 输出                             | 非 `HEARTBEAT_OK` 时投递 | 心跳提示 + 事件        | announce 摘要（默认）                               |
| [任务](/automation/tasks)        | 无任务记录               | 任务记录（静默）       | 任务记录（在 `openclaw tasks` 中可见）              |
...
- [后台任务](/automation/tasks) - 任务台账、审计和生命周期

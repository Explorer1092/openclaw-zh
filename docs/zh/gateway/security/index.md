---
title: "安全 🔒"
mmh3_hash: "3c3fd3c048654335cc1504e41f359737"
...
- 如果需要对抗用户隔离,按信��边界拆分(单独的 Gateway + 凭证,最好是单独的 OS 用户/主机)。
...
- 如果多个用户需要 OpenClaw,每个用户使用一个 VPS/主机。

### 实际后果(操作员信任边界)

在一个 Gateway 实例中,经过认证的操作员访问是受信任的控制平面角色,而不是每用户租户角色。

- 具有读/控制平面访问权限的操作员可以通过设计检查 Gateway Session 元数据/历史。
- Session 标识符(`sessionKey`、Session ID、标签)是路由选择器,而不是授权令牌。
- 示例:期望 `sessions.list`、`sessions.preview` 或 `chat.history` 等方法的每操作员隔离超出了此模型范围。
- 如果您需要对抗用户隔离,请按信任边界运行单独的 Gateway。
- 一台机器上的多个 Gateway 在技术上可行,但不是多用户隔离的推荐基线。

## 个人助手模型(不是多租户总线)

OpenClaw 被设计为个人助手安全模型:一个受信任的操作员边界,可能有许多 Agent。

- 如果几个人可以向一个启用工具的 Agent 发送消息,每个人都可以操纵相同的权限集。
- 每用户 Session/内存隔离有助于保护隐私,但不会将共享 Agent 转换为每用户主机授权。
- 如果用户可能相互对抗,请按信任边界运行单独的 Gateway(或单独的 OS 用户/主机)。
...
这些模式通常被报告,并且通常被关闭为无操作,除非显示了真实的边界绕过:

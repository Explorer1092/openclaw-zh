...
title: "配置 🔧"
sidebarTitle: "配置"
mmh3_hash: "e8c01211adf19134a0d09f3894c84157"
...
如果文件缺失,OpenClaw 使用相当安全的默认值 (embedded Pi agent + per-sender sessions + workspace `~/.openclaw/workspace`). You usually only need a config to:
- 限制谁可以触发 bot (`channels.whatsapp.allowFrom`, `channels.telegram.allowFrom`, etc.)
- 控制群组 allowlists + 提及行为 (`channels.whatsapp.groups`, `channels.telegram.groups`, `channels.discord.guilds`, `agents.list[].groupChat`)
- 自定义消息前缀 (`messages`)
- 设置 agent 的 workspace (`agents.defaults.workspace` or `agents.list[].workspace`)
- 调整嵌入式 agent 默认值 (`agents.defaults`) and session behavior (`session`)

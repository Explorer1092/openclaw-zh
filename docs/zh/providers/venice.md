...
| **匿名**       | 通过 Venice 代理，删除元数据。底层 Provider（OpenAI、Anthropic、Google、xAI）看到匿名请求。                                            | Claude、GPT、Gemini、Grok                                     |

<Warning>
匿名模型**不是**完全私密的。Venice 在转发前会删除元数据，但底层 Provider（OpenAI、Anthropic、Google、xAI）仍会处理请求。当需要完全隐私时，请选择**私密**模型。
</Warning>
...
<AccordionGroup>
  <Accordion title="私密模型（26 个）——完全私密，无日志记录">
    | 模型 ID                                | 名称                                | 上下文  | 功能                       |
    | -------------------------------------- | ----------------------------------- | ------- | -------------------------- |

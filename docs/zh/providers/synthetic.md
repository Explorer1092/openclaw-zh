...
Synthetic 公开 Anthropic 兼容的端点。OpenClaw 将其注册为 `synthetic` 提供商,并使用 Anthropic Messages API。
...
注意: OpenClaw 的 Anthropic 客户端将 `/v1` 附加到基础 URL,因此请使用
`https://api.synthetic.new/anthropic`(而不是 `/anthropic/v1`)。如果 Synthetic 更改其基础 URL,请覆盖 `models.providers.synthetic.baseUrl`。

## 模型目录

以下所有模型使用成本 `0`(输入/输出/缓存)。

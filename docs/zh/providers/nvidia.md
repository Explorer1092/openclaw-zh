---
mmh3_hash: "a806908c5d68b94a1bef11044da26a24"
...
<Warning>
如果您传递 `--nvidia-api-key` 而非环境变量，该值会出现在 Shell 历史和 `ps` 输出中。请尽可能优先使用 `NVIDIA_API_KEY` 环境变量。
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="慢速自定义 Provider 响应">
    某些 NVIDIA 托管的自定义模型在发出第一个响应块之前可能超过默认模型空闲看门狗时间。对于自定义 NVIDIA Provider 条目，请提高 Provider 超时时间，而不是提高整个 Agent 运行时超时时间：

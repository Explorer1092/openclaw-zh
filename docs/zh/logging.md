---
title: "日志记录"
mmh3_hash: "846605e2ab4bcfb90308de3980121ce1"
summary: "日志记录概述:文件日志、控制台输出、CLI 跟踪和控制 UI"
read_when: ["您需要日志记录的初学者友好概述","您想配置日志级别或格式","您正在故障排除并需要快速查找日志"]
---
...
工具摘要可以在敏感令牌到达控制台之前进行编辑:

- `logging.redactSensitive`:`off` | `tools`(默认:`tools`)
- `logging.redactPatterns`:正则表达式字符串列表以覆盖默认集

编辑**仅影响控制台输出**,不会改变文件日志。

## 诊断 + OpenTelemetry

诊断是结构化的、机器可读的事件,用于模型运行**和**消息流遥测(webhooks、排队、会话状态)。它们**不**替代日志;它们用于提供指标、跟踪和其他导出器。
...
使用标志在不提高 `logging.level` 的情况下打开额外的有针对性的调试日志。
...
- 遵守 `logging.level`(文件日志级别)。控制台编辑**不**适用于 OTLP 日志。

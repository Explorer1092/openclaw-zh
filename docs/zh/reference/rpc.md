---
mmh3_hash: "dad81ac4487fc2a31bffba201a287acb"
summary: "用于外部 CLI（signal-cli、imsg）和 Gateway 模式的 RPC 适配器"
read_when:
  - 添加或更改外部 CLI 集成
...
- OpenClaw 将 `imsg rpc` 作为子进程生成，用于 [iMessage](/channels/imessage)。
...
- Gateway 拥有进程（启动/停止与 Provider 生命周期绑定）。

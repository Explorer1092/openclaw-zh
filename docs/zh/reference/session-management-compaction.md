---
title: "会话管理与压缩 (深入研究)"
sidebarTitle: "会话管理与压缩"
mmh3_hash: "bb3631c2304e0bb69179c66ab38a1775"
summary: "深入研究: 会话存储 + 转录、生命周期和(自动)压缩内部"
read_when: ["您需要调试会话 id、转录 JSONL 或 sessions.json 字段","您正在更改自动压缩行为或添加\"压缩前\"维护","您想要实现内存刷新或静默系统轮次"]
---
# 会话管理与压缩 (深入研究)
...
- **每日重置**(默认凌晨 4:00 网关主机本地时间)在重置边界后的下一条消息上创建新的 `sessionId`。
- **空闲到期**(`session.reset.idleMinutes` 或遗留 `session.idleMinutes`)在消息在空闲窗口后到达时创建新的 `sessionId`。当每日 + 空闲都配置时,首先到期的获胜。
...
存储可以安全编辑,但网关是权威: 它可能会在会话运行时重写或重新水化条目。
...
目标: 在自动压缩发生之前,运行一个静默代理轮次,将持久状态写入磁盘(例如代理工作空间中的 `memory/YYYY-MM-DD.md`),以便压缩无法擦除关键上下文。

OpenClaw 使用**预阈值刷新**方法:

1) 监视会话上下文使用情况。
2) 当它越过"软阈值"(低于 Pi 的压缩阈值)时,运行静默
   "立即写入内存"指令到代理。
...
Pi 还在扩展 API 中公开 `session_before_compact` 钩子,但 OpenClaw 的刷新逻辑今天存在于网关端。
...
- 存储 vs 转录不匹配? 从 `openclaw status` 确认网关主机和存储路径。
- 压缩垃圾信息? 检查:
  - 模型上下文窗口(太小)

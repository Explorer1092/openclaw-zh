...
- **配置**(`models.providers.*.apiKey`、`tools.web.search.*`、`tools.web.fetch.firecrawl.*`、`memorySearch.*`、`talk.apiKey`)。
...
一些状态命令调用**提供程序使用端点**以显示配额窗口或身份验证健康。这些通常是低容量调用,但仍然命中提供程序 API:

...
<Steps>
  <Step title="安装本地 Node 运行时">
    下载固定的受支持 Node tarball（当前默认 `22.22.0`）到 `<prefix>/tools/node-v<version>` 并验证 SHA-256。
  </Step>
  <Step title="确保 Git">
...
<AccordionGroup>
  ...
  <Accordion title='Windows: "openclaw is not recognized"'>
    运行 `npm config get prefix`，将该目录添加到用户 PATH（Windows 上不需要 `\bin` 后缀），然后重新打开 PowerShell。
  </Accordion>

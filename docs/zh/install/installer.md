---
mmh3_hash: "3c9a41056fa467f32c6704ec304de7af"
...
| 脚本                               | 平台                 | 功能                                                                                                        |
| ---------------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL  | 如果需要则安装 Node，通过 npm（默认）或 git 安装 OpenClaw，并可以运行引导流程。                            |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | 将 Node + OpenClaw 安装到本地前缀（`~/.openclaw`），支持 npm 或 git checkout 模式。不需要 root。           |
| [`install.ps1`](#installps1)       | Windows (PowerShell) | 如果需要则安装 Node，通过 npm（默认）或 git 安装 OpenClaw，并可以运行引导流程。                            |
...
<Steps>
  <Step title="检测操作系统">
    支持 macOS 和 Linux（包括 WSL）。如果检测到 macOS，若 Homebrew 缺失则安装它。
  </Step>
  <Step title="确保默认安装 Node.js 24">
    检查 Node 版本并在需要时安装 Node 24（macOS 使用 Homebrew，Linux apt/dnf/yum 使用 NodeSource 设置脚本）。OpenClaw 仍支持 Node 22 LTS（当前为 `22.16+`）以保持兼容性。
  </Step>
  <Step title="确保 Git">
    如果 Git 缺失则安装。
  </Step>
  ...
  <Step title="安装后任务">
    ...
    - 在适当时尝试引导流程（TTY 可用、未禁用引导，且引导/配置检查通过）
...
如果没有可用的 TTY 且未设置安装方法，它默认为 `npm` 并发出警告。
...
<Info>
专为希望所有内容都在本地前缀（默认 `~/.openclaw`）下且不依赖系统 Node 的环境而设计。默认支持 npm 安装，以及在相同前缀流程下的 git-checkout 安装。
</Info>
...
<Steps>
  ...
  <Step title="在前缀下安装 OpenClaw">
    - `npm` 方法（默认）：使用 npm 安装在前缀下，然后将包装脚本写入 `<prefix>/bin/openclaw`
    - `git` 方法：克隆/更新 checkout（默认 `~/openclaw`），仍将包装脚本写入 `<prefix>/bin/openclaw`
...
<Steps>
  ...
  <Step title="确保默认安装 Node.js 24">
    如果缺失，则尝试通过 winget，然后 Chocolatey，然后 Scoop 安装。Node 22 LTS（当前为 `22.16+`）仍然受支持以保持兼容性。
  </Step>
  <Step title="安装 OpenClaw">
  ...
  <Step title="处理失败">
    `iwr ... | iex` 和 scriptblock 安装在不关闭当前 PowerShell 会话的情况下报告终止错误。直接 `powershell -File` / `pwsh -File` 安装仍然对自动化以非零方式退出。
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="为什么 npm 在 Linux 上遇到 EACCES？">
    某些 Linux 设置将 npm 全局前缀指向 root 拥有的路径。`install.sh` 可以将前缀切换到 `~/.npm-global` 并在 shell rc 文件中附加 PATH 导出（当这些文件存在时）。
  </Accordion>

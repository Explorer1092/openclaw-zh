---
mmh3_hash: "3ca06d741aa6f3441cec237b5889b484"
...
- **Node 24**（推荐）或 Node 22.14+ — 安装脚本自动处理
- **macOS、Linux 或 Windows** — 原生 Windows 和 WSL2 均受支持；WSL2 更稳定。参阅 [Windows](/platforms/windows)。
- 只有从源码构建时才需要 `pnpm`

## 推荐：安装程序脚本

最快的安装方式。它检测你的操作系统，如有需要则安装 Node，安装 OpenClaw，并启动引导。
...
当你希望 OpenClaw 和 Node 保存在本地前缀（如 `~/.openclaw`）下而不依赖系统范围的 Node 安装时使用：

```bash
curl -fsSL https://openclaw.ai/install-cli.sh | bash
```

它默认支持 npm 安装，加上相同前缀流程下的 git 检出安装。完整参考：[安装程序内部](/install/installer#install-clish)。

已安装？使用 `openclaw update --channel dev` 和 `openclaw update --channel stable` 在包安装和 git 安装之间切换。参阅
...
或跳过链接并在仓库内使用 `pnpm openclaw ...`。完整开发工作流请参阅 [设置](/start/setup)。
...
- 原生 Windows：首先使用计划任务，如果任务创建被拒绝则回退到每用户的启动文件夹登录项
...
如果 `$(npm prefix -g)/bin` 不在你的 `$PATH` 中，将其添加到你的 shell 启动文件（`~/.zshrc` 或 `~/.bashrc`）：

---
mmh3_hash: "78ce0a15815ebfb137ef3b8048c187c4"
title: "CI Pipeline"
...
| `check-additional`          | 架构、边界、导入循环保护以及 Gateway watch 回归测试                                        | Node 相关变更                |
| `build-smoke`               | 已构建 CLI 冒烟测试和启动内存冒烟测试                                                      | Node 相关变更                |
| `checks`                    | 剩余的 Linux Node 通道：Channel 测试和仅推送 Node 22 兼容性                               | Node 相关变更                |
...
4. 较重的平台和运行时通道随后展开：`checks-fast-core`、`checks-node-extensions`、`checks-node-core-test`、`extension-fast`、`checks`、`checks-windows`、`macos-node`、`macos-swift` 和 `android`。

...
Oracle 的免费层可能非常适合 OpenClaw（特别是如果你已经有 OCI 账户），但它有一些权衡：
...
使用 token 认证作为默认。它是可预测的，避免需要任何"不安全认证" Control UI 标志。

```bash
# 在 VM 上保持 Gateway 私有
openclaw config set gateway.bind loopback

# Gateway + Control UI 需要认证
...
这里的 `gateway.trustedProxies=["127.0.0.1"]` 仅用于本地 Tailscale Serve 代理的转发 IP/本地客户端处理。它**不是** `gateway.auth.mode: "trusted-proxy"`。此设置中的 diff viewer 路由保持失败关闭行为：没有转发代理标头的原始 `127.0.0.1` viewer 请求可能返回 `Diff not found`。如果需要可共享的 viewer 链接，请使用 `mode=file` / `mode=both` 附件，或有意启用远程 viewer 并设置 `plugins.entries.diffs.config.viewerBaseUrl`（或传递代理 `baseUrl`）。
...
现在一切正常，锁定 VCN 以阻止除 Tailscale 之外的所有流量。OCI 的虚拟云网络在网络边缘充当防火墙——流量在到达你的实例之前被阻止。
...
这会在网络边缘阻止端口 22 上的 SSH、HTTP、HTTPS 和其他所有内容。从现在起，你只能通过 Tailscale 连接。
...
VCN 锁定（仅 UDP 41641 打开）并且 Gateway 绑定到 loopback，你可以获得强大的纵深防御：公共流量在网络边缘被阻止，管理访问通过你的 tailnet 进行。

此设置通常消除了*需要*额外的基于主机的防火墙规则来阻止全球范围的 SSH 暴力破解的需要——但你仍应保持操作系统更新，运行 `openclaw security audit`，并验证你没有意外地在公共接口上侦听。

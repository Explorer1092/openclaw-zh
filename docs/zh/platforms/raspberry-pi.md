...
可修改安装可让你直接访问日志和代码 — 对调试 ARM 特定问题很有用。
...
大多数 OpenClaw 功能在 ARM64 上都能工作,但一些外部二进制文件可能需要 ARM 构建:
...
如果技能失败,请检查其二进制文件是否有 ARM 构建。许多 Go/Rust 工具有;有些没有。
...
由于 Pi 只是网关(模型在云中运行),使用基于 API 的模型:
...
| **Pi 5(8GB)** | 约 $80 | $0 | 过度但面向未来 |
| DigitalOcean | $0 | $6/月 | $72/年 |
| Hetzner | $0 | €3.79/月 | 约 $50/年 |

**收支平衡:** Pi 在约 6-12 个月内与云 VPS 相比就能回本。

...
mmh3_hash: "949da5f43eb906ee78cf226a43125584"
summary: "在 GCP Compute Engine VM(Docker)上全天候运行 OpenClaw 网关,具有持久状态"
read_when: ["你想在 GCP 上全天候运行 OpenClaw","你想要一个生产级、始终在线的网关在自己的 VM 上","你想要完全控制持久化、二进制文件和重启行为"]
---
...
使用 Docker 在 GCP Compute Engine VM 上运行持久的 OpenClaw 网关,具有持久状态、内置二进制文件和安全重启行为。

如果你想要"每月约 5-12 美元的全天候 OpenClaw",这是 Google Cloud 上的可靠设置。
定价因机器类型和区域而异;选择适合你工作负载的最小 VM,如果遇到 OOM 则扩容。
...
| e2-micro | 2 vCPU(共享),1GB RAM | 符合免费层资格 | 在负载下可能 OOM |
...
对于自动化或 CI/CD 管道,使用最小权限创建专用服务账户:

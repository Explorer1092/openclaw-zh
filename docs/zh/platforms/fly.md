---
mmh3_hash: "ba49aa6fbca87e0a5b733fa944d082bb"
title: "Fly.io 部署"
sidebarTitle: "Fly.io"
...
**目标:** OpenClaw 网关在 [Fly.io](https://fly.io) 机器上运行,具有持久存储、自动 HTTPS 和 Discord/通道访问。
...
- 通道凭据:Discord 机器人令牌、Telegram 令牌等
...
**安全说明:** 默认配置公开一个公共 URL。对于无公共 IP 的强化部署,请参阅[私有部署](#private-deployment-hardened)或使用 `fly.private.toml`。
...
- 像对待密码一样对待这些令牌。
- **优先使用环境变量而非配置文件**来存储所有 API 密钥和令牌。这样可以将密钥保留在 `openclaw.json` 之外,避免意外暴露或记录。
...
```bash
mkdir -p /data
cat > /data/openclaw.json << 'INNER_EOF'
{
  "agents": {
    "defaults": {
      "model": {
...
如果使用环境变量,无需将令牌添加到配置。网关会自动读取 `DISCORD_BOT_TOKEN`。
...
粘贴你的网关令牌(来自 `OPENCLAW_GATEWAY_TOKEN` 的那个)进行认证。
...
**注意:** 512MB 太小。1GB 可能有效,但在负载或详细日志记录下可能 OOM。**推荐 2GB。**
...
如果使用 `--allow-unconfigured`,网关会创建最小配置。重启后应读取 `/data/openclaw.json` 的自定义配置。
...
**注意:** `fly deploy` 后,机器命令可能会重置为 `fly.toml` 中的内容。如果你进行了手动更改,请在部署后重新应用。
...
之后,`fly ips list` 应仅显示 `private` 类型的 IP:
```
...
3. **仅出站** - 一些提供商(Twilio)在没有 webhook 的情况下对出站调用也能正常工作
...
ngrok 隧道在容器内运行,并提供公共 webhook URL,而不暴露 Fly 应用本身。

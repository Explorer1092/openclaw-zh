...
read_when:
  ...
  - 修改 Pi 的 agent 会话生命周期、工具或 Provider 连接
---

OpenClaw 与 [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) 及其兄弟包（`pi-ai`、`pi-agent-core`、`pi-tui`）集成，以驱动其 AI agent 功能。

## 概述

OpenClaw 使用 pi SDK 将 AI 编码 agent 嵌入其消息 Gateway 架构中。OpenClaw 不是将 pi 作为子进程生成或使用 RPC 模式，而是通过 `createAgentSession()` 直接导入并实例化 pi 的 `AgentSession`。这种嵌入式方法提供：

- 对会话生命周期和事件处理的完全控制
- 自定义工具注入（消息、沙箱、Channel 特定操作）
- 每个 Channel/上下文的系统提示定制
- 支持分支/压缩的会话持久化
- 具有故障转移的多账户认证配置文件轮换
- Provider 无关的模型切换
...
│   ├── extra-params.ts            # Provider 特定的流式传输参数
...
├── pi-embedded-block-chunker.ts   # 流式传输块回复分块
...
├── auth-profiles.ts               # 配置文件存储、冷却时间、故障转移
...
├── sandbox.ts                     # 沙箱上下文解析
├── sandbox/                       # 沙箱子系统
├── channel-tools.ts               # Channel 特定工具注入
...
Channel 特定的消息操作运行时现在位于 Plugin 拥有的扩展目录中，而不是在 `src/agents/tools` 下，例如：
...
- `message_start` / `message_end` / `message_update`（流式传输文本/思考）
...
SDK 处理完整的 agent 循环：发送给 LLM、执行工具调用、流式传输响应。

图像注入是提示本地的：OpenClaw 从当前提示加载图像引用，并仅为该轮次通过 `images` 传递。它不会重新扫描较旧的历史轮次以重新注入图像负载。
...
2. **自定义替换**：OpenClaw 用 `exec`/`process` 替换 bash，为沙箱自定义读取/编辑/写入工具
3. **OpenClaw 工具**：消息、浏览器、Canvas、会话、Cron、Gateway 等
4. **Channel 工具**：Discord/Telegram/Slack/WhatsApp 特定操作工具
5. **策略过滤**：按配置文件、Provider、agent、群组、沙箱策略过滤工具
6. **Schema 规范化**：为 Gemini/OpenAI 特性清理 Schema
7. **AbortSignal 包装**：包装工具以遵守中止信号

### 工具定义适配器

pi-agent-core 的 `AgentTool` 与 pi-coding-agent 的 `ToolDefinition` 的 `execute` 签名不同。`pi-tool-definition-adapter.ts` 中的适配器桥接了这一差异：
...
这确保 OpenClaw 的策略过滤、沙箱集成和扩展工具集在 Provider 之间保持一致。

## 系统提示构建

系统提示在 `buildAgentSystemPrompt()`（`system-prompt.ts`）中构建。它组装了一个完整的提示，其中包含工具、工具调用样式、安全防护、OpenClaw 控制、Skill、文档、工作区、沙箱、消息、助手输出指令、语音、静默回复、心跳、运行时元数据等部分，以及启用时的记忆和反应，以及可选的上下文文件和额外系统提示内容。子 agent 使用的最小提示模式会裁剪各部分。
...
`limitHistoryTurns()` 根据 Channel 类型（DM 与群组）裁剪对话历史。

### 压缩

自动压缩在上下文溢出时触发。常见的溢出特征包括 `request_too_large`、`context length exceeded`、`input exceeds the maximum number of tokens`、`input token count exceeds the maximum number of input tokens`、`input is too long for the model` 和 `ollama error: context length exceeded`。`compactEmbeddedPiSessionDirect()` 处理手动压缩：
...
OpenClaw 维护一个每个 Provider 具有多个 API 密钥的认证配置文件存储：
...
`EmbeddedBlockChunker` 将流式传输文本管理为离散的回复块：
...
流式传输输出经过处理以剥离 `<think>`/`<thinking>` 块并提取 `<final>` 内容：
...
| 系统提示          | AGENTS.md + 提示         | 每个 Channel/上下文动态生成                                                                    |
...
4. **流式传输处理器复杂性**：`subscribeEmbeddedPiSession` 已经变得很大
5. **Provider 特性**：许多 Provider 特定的代码路径，pi 可能可以处理

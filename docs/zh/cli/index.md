...
| 模型与推理 | [`models`](/cli/models) · [`infer`](/cli/infer) · `capability`（[`infer`](/cli/infer) 的别名）· [`memory`](/cli/memory) · [`wiki`](/cli/wiki)                                                                          |
| 网络与 Node    | [`directory`](/cli/directory) · [`nodes`](/cli/nodes) · [`devices`](/cli/devices) · [`node`](/cli/node)                                                                                                                                   |
| 运行时与 Sandbox  | [`approvals`](/cli/approvals) · `exec-policy`（参见 [`approvals`](/cli/approvals)）· [`sandbox`](/cli/sandbox) · [`tui`](/cli/tui) · `chat`/`terminal`（[`tui --local`](/cli/tui) 的别名）· [`browser`](/cli/browser)                 |
...
- `/config` — 持久化配置更改。
- `/debug` — 仅运行时的配置覆盖（内存，而非磁盘；需要 `commands.debug: true`）。

## 使用跟踪

`openclaw status --usage` 和 Control UI 在 OAuth/API 凭据可用时显示 Provider 使用/配额。数据直接来自 Provider 使用端点，并规范化为 `X% left`。当前有使用窗口的 Provider：Anthropic、GitHub Copilot、Gemini CLI、OpenAI Codex、MiniMax、Xiaomi 和 z.ai。

详细信息请参见[使用跟踪](/concepts/usage-tracking)。

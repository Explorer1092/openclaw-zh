...
`openclaw exec-policy` 是一步同时保持请求的 `tools.exec.*` 配置和本地主机批准文件对齐的本地便捷命令。

在以下情况下使用它：

- 检查本地请求的策略、主机批准文件和有效合并
...
- `--host node` 在此命令中被拒绝，因为节点 exec 批准在运行时从节点获取，必须通过针对节点的批准命令来管理
...
该本地快捷方式同时更新请求的本地 `tools.exec.*` 配置和本地批准默认值。它在意图上等同于上述手动两步设置，但仅适用于本地机器。

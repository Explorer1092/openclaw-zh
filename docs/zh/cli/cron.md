---
mmh3_hash: "3170ec08047751aee91351bafbff2dc1"
...
隔离 cron 运行以来自嵌入运行的结构化执行拒绝元数据作为权威的拒绝信号。当嵌套结构化错误消息以 `SYSTEM_RUN_DENIED` 或 `INVALID_REQUEST` 开头时，它们也遵从节点主机的 `UNAVAILABLE` 包装。

Cron 不会将最终输出的普通文本或看起来像审批的拒绝短语分类为拒绝，除非嵌入运行也提供了结构化拒绝元数据，因此普通助手文本不会被视为被阻止的命令。

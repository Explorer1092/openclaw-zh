...
OpenClaw 默认对传输时间戳使用**主机本地时间**,并且仅在系统提示中使用**用户时区**。
提供程序时间戳被保留,因此工具保持其原生语义(当前时间可通过 `session_status` 获得)。
...
- 使用显式 IANA 时区(例如,`"America/Chicago"`)来固定时区。
...
如果已知用户时区,系统提示包含一个专用的**当前日期和时间**部分,**仅包含时区**(没有时钟/时间格式)以保持提示缓存稳定:
...
插入到代理上下文中的排队系统事件带有时间戳前缀,使用与消息信封相同的时区选择(默认:主机本地)。
...
通道工具返回**提供程序原生时间戳**并添加规范化字段以保持一致性:

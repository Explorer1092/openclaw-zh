...
read_when: ["将 Gmail 收件箱触发器接入 OpenClaw 时","设置 Pub/Sub 推送以进行智能体唤醒时"]
---
...
如果你想要固定的频道，设置 `channel` + `to`。否则 `channel: "last"` 使用最后的传递路由（回退到 WhatsApp）。
...
如果你需要后端接收带前缀的路径，请将 `hooks.gmail.tailscale.target` (或 `--tailscale-target`) 设置为完整的 URL，如 `http://127.0.0.1:8788/gmail-pubsub` 并匹配 `hooks.gmail.serve.path`。
...
1) 选择 **拥有 OAuth 客户端**（`gog` 使用的）的 GCP 项目。

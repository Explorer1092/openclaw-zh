---
mmh3_hash: "984219c7d5759c439cbde0fdac7db428"
...
1. 模型范围的运行时策略获胜。
2. Provider 范围的运行时策略次之。
3. `auto` 询问已注册的测试框架是否支持解析的 Provider/模型。
...
整个 Session 和整个 Agent 的运行时固定被选择忽略。这包括过时的 Session `agentHarnessId` 值、`agents.defaults.agentRuntime`、`agents.list[].agentRuntime` 和 `OPENCLAW_AGENT_RUNTIME`。`/status` 显示从 Provider/模型路由选择的有效运行时。如果选择的测试框架令人意外，请启用 `agents/harness` 调试日志记录并检查 Gateway 的结构化 `agent harness selected` 记录。它包括选定的测试框架 id、选择原因、运行时/回退策略，以及在 `auto` 模式下每个 Plugin 候选的支持结果。
...
Codex Plugin 是附加的。官方 OpenAI Provider 上的普通 `openai/gpt-*` Agent 引用默认选择 Codex 测试框架。旧版 `openai-codex/*` 路由应使用 `openclaw doctor --fix` 修复，旧版 `codex/*` 模型引用仍然是原生测试框架的兼容性别名。
...
捆绑的 `codex` 测试框架是嵌入式 OpenClaw Agent 轮次的原生 Codex 模式。首先启用捆绑的 `codex` Plugin，如果您的配置使用限制性允许列表，则在 `plugins.allow` 中包含 `codex`。原生应用服务器配置应使用 `openai/gpt-*`；OpenAI Agent 轮次默认选择 Codex 测试框架。旧版 `openai-codex/*` 路由应使用 `openclaw doctor --fix` 修复，旧版 `codex/*` 模型引用仍然是原生测试框架的兼容性别名。

当此模式运行时，Codex 拥有原生线程 id、恢复行为、压缩和应用服务器执行。OpenClaw 仍然拥有聊天 Channel、可见转录镜像、工具策略、审批、媒体投递和 Session 选择。当您需要证明只有 Codex 应用服务器路径可以声明运行时，使用 Provider/模型 `agentRuntime.id: "codex"`。显式 Plugin 运行时默认已失败关闭；Codex 应用服务器选择失败和运行时失败不会通过 PI 重试。

## 运行时严格性

默认情况下，OpenClaw 使用 `auto` Provider/模型运行时策略：已注册的 Plugin 测试框架可以声明 Provider/模型对，PI 在没有匹配时处理轮次。官方 OpenAI Provider 上的 OpenAI Agent 引用默认为 Codex。当缺失的测试框架选择应该失败而不是路由到 PI 时，使用显式的 Provider/模型 Plugin 运行时（如 `agentRuntime.id: "codex"`）。选定的 Plugin 测试框架失败总是硬失败。这不会阻止显式的 Provider/模型 `agentRuntime.id: "pi"`。
...
如果您希望为一个规范模型使用 CLI 后端，将运行时放在该模型条目上：
...
使用显式 Plugin 运行时时，当请求的测试框架未注册、不支持解析的 Provider/模型或在产生轮次副作用之前失败时，Session 会提前失败。这是仅 Codex 部署和必须证明 Codex 应用服务器路径实际在使用的实时测试的有意设计。

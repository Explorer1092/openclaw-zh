---
mmh3_hash: "5d72e9b288f112cd0297adb6eb12da4c"
title: "Diffs"
summary: "供 Agent 使用的只读 diff 查看器与 PNG 渲染器（可选插件工具）"
description: "使用可选的 Diffs 插件，将前后文本或统一格式的 patch 渲染为 Gateway 托管的 diff 查看页面、PNG 图片，或两者兼得。"
read_when:
  - 希望 Agent 以 diff 形式展示代码或 Markdown 的修改内容
  - 需要适用于 Canvas 的查看器 URL 或渲染后的 diff PNG 图片
  - 需要具有安全默认设置的受控、临时 diff 产物
---

# Diffs

`diffs` 是一个可选的插件工具，可将变更内容转换为供 Agent 使用的只读 diff 产物。
...
3. 对于以聊天/图片为主的流程，使用 `mode: "image"` 调用 `diffs`。
...
3. Agent 执行以下操作之一：
   ...
   - 使用 `path` 或 `filePath` 通过 `message` 发送 `details.imagePath`
   - 同时执行以上两项
...
- `imagePath`
- `path`（与 `imagePath` 值相同，用于兼容 message 工具）
- `imageBytes`

模式行为说明：

- `mode: "view"`：仅返回查看器字段。
- `mode: "image"`：仅返回图片字段，不创建查看器产物。
- `mode: "both"`：同时返回查看器字段和图片字段。若截图失败，查看器仍会返回，并附带 `imageError`。
...
`mode: "image"` 和 `mode: "both"` 需要 Chromium 兼容浏览器。
...
- 需要附件的出站聊天 Channel 首选 `mode: "image"`。

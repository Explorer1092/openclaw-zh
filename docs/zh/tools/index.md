---
title: "工具(OpenClaw)"
mmh3_hash: "faf4c03c746f1b9020a6851dd148af06"
...
OpenClaw 为浏览器、画布、节点和 cron 公开**一流的 agent 工具**。这些替代了旧的 `openclaw-*` 技能: 工具是类型化的,无需 shell,agent 应该直接依赖它们。

## 禁用工具

您可以通过 `openclaw.json` 中的 `tools.allow` / `tools.deny` 全局允许/拒绝工具(拒绝获胜)。这可以防止不允许的工具被发送到模型提供商。
...
使用 `tools.byProvider` 为特定提供商(或单个 `provider/model`)**进一步限制**工具,而无需更改全局默认值。每个 agent 覆盖: `agents.list[].tools.byProvider`。
...
插件可以注册超出核心集的**附加工具**(和 CLI 命令)。有关安装 + 配置,请参见 [插件](/plugin),有关如何将工具使用指导注入提示的信息,请参见 [技能](/tools/skills)。一些插件在工具旁边提供自己的技能(例如,voice-call 插件)。

可选插件工具:
- [Lobster](/tools/lobster): 带有可恢复批准的类型化工作流运行时(需要网关主机上的 Lobster CLI)。
...
- `process` 的作用域限定为每个 agent;来自其他 agent 的会话不可见。

---
mmh3_hash: "2456505012d93be84f50838d1ac8bcc5"
...
对于异步工具，OpenClaw 将请求提交给提供商，立即返回任务 id，并在任务账本中跟踪作业。Agent 在作业运行期间继续响应其他消息。当提供商完成时，OpenClaw 唤醒 Agent，并携带生成的媒体路径，以便 Agent 告知用户并通过 message 工具传递结果。如果请求者 Session 已不活跃且部分生成的媒体仍未通过 message 工具投递，OpenClaw 会以幂等方式直接发送仅包含缺失媒体的回退投递。已通过 message 工具投递的媒体不会再次发布。

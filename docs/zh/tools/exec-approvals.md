...
mmh3_hash: "c8abdadf4dc668a85659fce5c411e903"
...
这些在节点报告事件后发布到 Agent Session。被拒绝的 exec 审批是终态：OpenClaw 可以向操作者或直接聊天路由报告拒绝，但不会将 `Exec denied` 发回 Agent Session 或唤醒 Agent 工作。Gateway 主机 exec 审批在命令完成时（以及可选地在运行时间超过阈值时）发出相同的生命周期事件。受审批门控的 exec 将审批 id 复用为这些消息中的 `runId` 以便于关联。

## 被拒绝审批的行为

当异步 exec 审批被拒绝时，OpenClaw 将该请求视为终态。它可以向操作者或直接聊天路由显示简洁的拒绝信息，但不会通过 Agent Session 发回拒绝指导。这防止被拒绝的命令成为另一个模型轮次，并阻止 Agent 重用同一命令早期运行的输出。

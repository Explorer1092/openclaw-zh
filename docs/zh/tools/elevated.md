...
- 仅在 agent **被沙箱化**时更改行为(否则 exec 已在主机上运行)。
...
- 每个 agent 的门控: `agents.list[].tools.elevated.enabled`(可选;只能进一步限制)。
- 每个 agent 的允许列表: `agents.list[].tools.elevated.allowFrom`(可选;设置时,发送者必须匹配**全局 + 每个 agent 的**允许列表)。
- Discord 回退: 如果省略 `tools.elevated.allowFrom.discord`,则使用 `channels.discord.dm.allowFrom` 列表作为回退。设置 `tools.elevated.allowFrom.discord`(即使是 `[]`)以覆盖。每个 agent 的允许列表**不**使用回退。

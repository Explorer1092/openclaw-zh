...
title: "斜杠命令"
sidebarTitle: "斜杠命令"
mmh3_hash: "4a7238af42bfb7a2a3c581e01d168f32"
summary: "斜杠命令: 文本 vs 原生、配置和支持的命令"
read_when: ["使用或配置聊天命令","调试命令路由或权限"]
---
# 斜杠命令

命令由网关处理。大多数命令必须作为以 `/` 开头的**独立**消息发送。仅主机的 bash 聊天命令使用 `! <cmd>`(以 `/bash <cmd>` 作为别名)。
...
- **指令**: `/think`、`/verbose`、`/reasoning`、`/elevated`、`/exec`、`/model`、`/queue`。
  ...
  - 指令仅对**授权发送者**(频道允许列表/配对加 `commands.useAccessGroups`)应用。未授权的发送者看到指令被视为纯文本。
...
- `commands.native`(默认 `"auto"`)注册原生命令。
  - 自动: 对 Discord/Telegram 开启;对 Slack 关闭(直到您添加斜杠命令);对不支持的提供商忽略。
  - 设置 `channels.discord.commands.native`、`channels.telegram.commands.native` 或 `channels.slack.commands.native` 以每个提供商覆盖(bool 或 `"auto"`)。
  - `false` 在启动时清除 Discord/Telegram 上先前注册的命令。Slack 命令在 Slack 应用中管理,不会自动删除。
- `commands.nativeSkills`(默认 `"auto"`)在支持时原生注册**技能**命令。
  - 自动: 对 Discord/Telegram 开启;对 Slack 关闭(Slack 需要为每个技能创建斜杠命令)。
  - 设置 `channels.discord.commands.nativeSkills`、`channels.telegram.commands.nativeSkills` 或 `channels.slack.commands.nativeSkills` 以每个提供商覆盖(bool 或 `"auto"`)。
...
- 有关完整的提供商使用细分,使用 `openclaw status --usage`。
...
- **技能命令:** `user-invocable` 技能作为斜杠命令公开。名称清理为 `a-z0-9_`(最多 32 个字符);冲突获得数字后缀(例如 `_2`)。
  - `/skill <name> [input]` 按名称运行技能(当原生命令限制阻止每个技能命令时很有用)。
  - 默认情况下,技能命令作为正常请求转发到模型。
  - 技能可以可选地声明 `command-dispatch: tool` 以将命令直接路由到工具(确定性,无模型)。
...
- **提供商使用/配额**(示例: "Claude 剩余 80%")在当前模型提供商启用使用跟踪时显示在 `/status` 中。

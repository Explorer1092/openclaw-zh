---
mmh3_hash: "eda7a9286bf1db1c55d4169a777495b9"
...
- 相同的 Session 上下文
- 单独的一次性旁查询
- Session 使用原生 harness 时使用相同的原生 harness 传输
- 不污染未来上下文
- 不持久化到对话记录

对于 Codex harness Session，BTW 通过将活跃应用服务器线程作为临时旁线程进行分叉来保持在 Codex 内部。这使 Codex OAuth 和原生线程行为保持完整，同时仍将旁答案与父对话记录隔离。与 Codex `/side` 一样，旁线程保持当前 Codex 权限和原生工具界面，并通过护栏告知模型不要将继承的父线程工作视为活跃指令。非 Codex 运行时保留旧版直接一次性路径。

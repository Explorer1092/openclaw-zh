...
title: "位置命令"
sidebarTitle: "位置命令"
mmh3_hash: "6b1e66093ea17a0b7fcac191ae714ba9"
summary: "Node 的位置命令 (location.get)，权限模式和后台行为"
read_when: ["添加位置 Node 支持或权限 UI 时","设计后台位置 + 推送流程时"]
---
...
1. Gateway 向 Node 发送推送（静默推送或 FCM 数据）。
2. Node 短暂唤醒并向设备请求位置。
3. Node 将载荷转发给 Gateway。

...
- MIME 检测优先考虑魔术字节，然后是标头，最后是文件扩展名。
...
- 当启用每会话 Docker 沙盒时，入站媒体会被复制到沙盒工作区，`MediaPath`/`MediaUrl` 重写为类似 `media/inbound/<filename>` 的相对路径。

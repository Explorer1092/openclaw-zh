...
read_when: "您希望在多代理网关中实现每个代理的沙箱或每个代理的工具允许/拒绝策略。"
status: active
---
...
身份验证是每个代理的:每个代理从其自己的 `agentDir` 身份验证存储中读取:

```
~/.openclaw/agents/<agentId>/agent/auth-profiles.json
```

凭证**不**在代理之间共享。切勿跨代理重用 `agentDir`。
如果您想共享凭证,请将 `auth-profiles.json` 复制到另一个代理的 `agentDir` 中。

有关沙箱在运行时的行为方式,请参阅[沙箱](/gateway/sandboxing)。
有关调试"为什么被阻止?",请参阅[沙箱 vs 工具策略 vs 提升](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `openclaw sandbox explain`。
...
2. **提供程序工具配置文件**(`tools.byProvider[provider].profile` 或 `agents.list[].tools.byProvider[provider].profile`)
3. **全局工具策略**(`tools.allow` / `tools.deny`)
4. **提供程序工具策略**(`tools.byProvider[provider].allow/deny`)
5. **代理特定工具策略**(`agents.list[].tools.allow/deny`)
6. **代理提供程序策略**(`agents.list[].tools.byProvider[provider].allow/deny`)
7. **沙箱工具策略**(`tools.sandbox.tools` 或 `agents.list[].tools.sandbox.tools`)
8. **子代理工具策略**(`tools.subagents.tools`,如果适用)

每个级别都可以进一步限制工具,但不能授予先前级别拒绝的工具。
...
- `group:nodes`: `nodes`
- `group:openclaw`: 所有内置 OpenClaw 工具(不包括提供程序插件)
...
- 为敏感配置文件禁用每个代理的提升(`agents.list[].tools.elevated.enabled: false`)
...
旧的 `agent.*` 配置由 `openclaw doctor` 迁移;今后优先使用 `agents.defaults` + `agents.list`。
...
`agents.defaults.sandbox.mode: "non-main"` 基于 `session.mainKey`(默认 `"main"`),而不是代理 id。群组/通道会话始终获取自己的键,因此它们被视为非 main 并将被沙箱化。如果您希望代理永不沙箱,请设置 `agents.list[].sandbox.mode: "off"`。

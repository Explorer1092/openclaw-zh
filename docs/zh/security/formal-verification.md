...
通过在本地克隆模型仓库并运行 TLC 来重现结果（见下文）。未来的迭代可能提供：
...
**声明：** 超出 loopback 绑定而不进行身份验证可能使远程攻击成为可能 / 增加暴露面；令牌/密码阻止未经身份验证的攻击者（根据模型假设）。
...
**声明：** `exec host=node` 需要（a）node 命令允许列表加上声明的命令和（b）配置时的实时批准；批准被令牌化以防止重放（在模型中）。
...
**声明：** 来自不同对等方的 DM 不会合并到同一 session 中，除非明确链接/配置。
...
- 在并发请求下，你不能超过 channel 的 `MaxPending`。
...
**声明：** 摄取应该在扇出中保留跟踪关联，并在 provider 重试下是幂等的。

含义：

- 当一个外部事件变成多个内部消息时，每个部分都保持相同的跟踪/事件身份。
- 重试不会导致双重处理。
- 如果 provider 事件 ID 缺失，去重会回退到安全密钥（例如跟踪 ID）以避免丢弃不同的事件。
...
**声明：** 路由必须默认保持 DM session 隔离，并且仅在明确配置时合并 session（channel 优先级 + 身份链接）。

含义：

- channel 特定的 dmScope 覆盖必须优先于全局默认值。
- identityLinks 应该仅在显式链接组内合并，而不是跨不相关的对等方。

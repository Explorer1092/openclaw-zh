---
mmh3_hash: "a242a03c45fd6759267cc48d259a3e5e"
...
**框架：** [MITRE ATLAS](https://atlas.mitre.org/)（AI 系统的对抗性威胁图谱）

### 框架归属

这个威胁模型基于 [MITRE ATLAS](https://atlas.mitre.org/)，这是记录 AI/ML 系统对抗性威胁的行业标准框架。ATLAS 由 [MITRE](https://www.mitre.org/) 与 AI 安全社区合作维护。
...
这是由 OpenClaw 社区维护的活动文档。参见 [CONTRIBUTING-THREAT-MODEL.md](/security/CONTRIBUTING-THREAT-MODEL) 了解贡献指南：
...
本威胁模型使用专为 AI/ML 系统设计的 MITRE ATLAS 框架，记录对 OpenClaw AI Agent 平台和 ClawHub 技能市场的对抗性威胁。

### 1.2 范围

| 组件                   | 是否包含 | 备注                                             |
| ---------------------- | -------- | ------------------------------------------------ |
| OpenClaw Agent 运行时  | 是       | 核心 Agent 执行、工具调用、Session               |
| Gateway                | 是       | 认证、路由、Channel 集成                         |
| Channel 集成           | 是       | WhatsApp、Telegram、Discord、Signal、Slack 等    |
| ClawHub 市场           | 是       | 技能发布、审核、分发                             |
| MCP 服务器             | 是       | 外部工具 Provider                                |
| 用户设备               | 部分     | 移动应用、桌面客户端                             |
...
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                  执行沙盒                                 │   │
│  │  • Docker 沙盒 OR 主机（exec-approvals）                  │   │
│  │  • Node 远程执行                                          │   │
│  │  • SSRF 保护（DNS 固定 + IP 封锁）                        │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
| F2   | Gateway | Agent   | 路由消息           | Session 隔离         |
| F3   | Agent   | 工具    | 工具调用           | 策略执行             |
| F4   | Agent   | 外部    | web_fetch 请求     | SSRF 封锁            |
...
| **残余风险**     | 中——公开 Gateway 可被发现                                             |
| **建议**         | 记录安全部署文档，在发现端点上添加速率限制                           |
...
| ---------------- | ---------------------------------------------------------------- |
| **ATLAS ID**     | AML.T0006 - 主动扫描                                             |
| **描述**         | 攻击者探测消息 Channel 以识别 AI 管理的账号                      |
| **攻击向量**     | 发送测试消息、观察响应模式                                       |
| **受影响组件**   | 所有 Channel 集成                                                |
| **当前缓解措施** | 无特定措施                                                       |
| **残余风险**     | 低——仅发现的价值有限                                              |
...
| **描述**         | 攻击者在配对宽限期内拦截配对码（DM Channel 配对 1 小时，节点配对 5 分钟）                                    |
| **攻击向量**     | 窥探、网络嗅探、社会工程                                                                                      |
| **受影响组件**   | 设备配对系统                                                                                                  |
| **当前缓解措施** | 1 小时过期（DM 配对）/ 5 分钟过期（节点配对），码通过现有 Channel 发送                                       |
| **残余风险**     | 中——宽限期可被利用                                                                                            |
| **建议**         | 缩短宽限期，添加确认步骤                                                                                      |
...
| ---------------- | ---------------------------------------------------------------------------- |
| **ATLAS ID**     | AML.T0040 - AI 模型推理 API 访问                                              |
| **描述**         | 攻击者在 Channel 中欺骗允许的发件人身份                                      |
| **攻击向量**     | 取决于 Channel——电话号码欺骗、用户名冒充                                     |
| **受影响组件**   | 每个 Channel 的 AllowFrom 验证                                               |
| **当前缓解措施** | Channel 特定的身份验证                                                       |
| **残余风险**     | 中——某些 Channel 容易受到欺骗                                                |
| **建议**         | 记录 Channel 特定风险，在可能的地方添加加密验证                              |
...
| **描述**         | 攻击者发送精心构造的提示词来操纵 Agent 行为                                              |
| **攻击向量**     | 包含对抗性指令的 Channel 消息                                                             |
| **受影响组件**   | Agent LLM、所有输入面                                                                    |
| **当前缓解措施** | 模式检测、外部内容包裹                                                                   |
| **残余风险**     | 严重——仅检测，无封锁；复杂攻击会绕过                                                     |
...
| **描述**         | 攻击者在获取的内容中嵌入恶意指令                            |
| **攻击向量**     | 恶意 URL、中毒邮件、被入侵的 Webhook                        |
| **受影响组件**   | web_fetch、邮件摄取、外部数据源                             |
| **当前缓解措施** | 使用 XML 标签和安全提示的内容包裹                           |
| **残余风险**     | 高——LLM 可能忽略包裹指令                                    |
| **建议**         | 实施内容清理，分离执行上下文                                |
...
| **攻击向量**     | 创建账号，发布包含隐藏恶意代码的技能                                     |
| **受影响组件**   | ClawHub、技能加载、Agent 执行                                            |
| **当前缓解措施** | GitHub 账号年龄验证、基于模式的审核标记                                  |
| **残余风险**     | 严重——无沙盒，审查有限                                                   |
| **建议**         | VirusTotal 集成（进行中）、技能沙盒、社区审查                            |
...
#### T-DISC-002：Session 数据提取

| 属性             | 值                                                    |
| ---------------- | ----------------------------------------------------- |
| **ATLAS ID**     | AML.T0040 - AI 模型推理 API 访问                       |
| **描述**         | 攻击者从 Session 上下文中提取敏感数据                 |
| **攻击向量**     | "我们讨论了什么？"查询、上下文探测                   |
| **受影响组件**   | Session 转录、上下文窗口                              |
| **当前缓解措施** | 每个发件人的 Session 隔离                             |
| **残余风险**     | 中——Session 内数据可访问                              |
| **建议**         | 在上下文中实施敏感数据脱敏                            |

---

### 3.7 收集与数据渗漏（AML.TA0009、AML.TA0010）
...
| **攻击向量**     | 导致 Agent 将数据 POST 到攻击者服务器的提示词注入                      |
| **受影响组件**   | web_fetch 工具                                                         |
| **当前缓解措施** | 内部网络的 SSRF 封锁                                                   |
| **残余风险**     | 高——允许外部 URL                                                       |
| **建议**         | 实施 URL 允许列表、数据分类感知                                        |

#### T-EXFIL-002：未授权的消息发送

| 属性             | 值                                                             |
| ---------------- | -------------------------------------------------------------- |
| **ATLAS ID**     | AML.T0009 - 收集                                               |
| **描述**         | 攻击者导致 Agent 发送包含敏感数据的消息                        |
| **攻击向量**     | 导致 Agent 向攻击者发消息的提示词注入                          |
| **受影响组件**   | Message 工具、Channel 集成                                     |
| **当前缓解措施** | 出站消息门控                                                   |
| **残余风险**     | 中——门控可能被绕过                                             |
| **建议**         | 对新收件人要求明确确认                                         |
...
| ---------------- | ------------------------------------------------------- |
| **ATLAS ID**     | AML.T0009 - 收集                                        |
| **描述**         | 恶意技能从 Agent 上下文中收割凭证                       |
| **攻击向量**     | 技能代码读取环境变量、配置文件                          |
| **受影响组件**   | 技能执行环境                                            |
| **当前缓解措施** | 无技能特定措施                                          |
| **残余风险**     | 严重——技能以 Agent 权限运行                             |
| **建议**         | 技能沙盒、凭证隔离                                      |
...
| **攻击向量**     | 自动消息洪泛、昂贵的工具调用                     |
| **受影响组件**   | Gateway、Agent Session、API Provider             |
| **当前缓解措施** | 无                                               |
| **残余风险**     | 高——无速率限制                                   |
| **建议**         | 实施每发件人速率限制、成本预算                   |
...
| **攻击向量**     | 导致不当响应的提示词注入                                |
| **受影响组件**   | 输出生成、Channel 消息                                  |
| **当前缓解措施** | LLM Provider 内容策略                                   |
| **残余风险**     | 中——Provider 过滤不完善                                 |
| **建议**         | 输出过滤层、用户控制                                    |
...
| 必填 SKILL.md      | 必要的 readme               | 安全价值低——仅提供信息                                       |
| 模式审核           | moderation.ts 中的 FLAG_RULES | 低——容易被绕过                                             |
| 审核状态           | `moderationStatus` 字段     | 中——可进行手动审查                                           |
...
- 仅检查 slug、displayName、summary、frontmatter、元数据、文件路径
- 不分析实际技能代码内容
- 简单正则表达式容易通过混淆绕过
- 无行为分析
...
| R-002 | 实施技能沙盒                      | T-PERSIST-001、T-EXFIL-003 |
...
| ID    | 建议                                          | 解决          |
| ----- | --------------------------------------------- | ------------- |
| R-008 | 在可能的地方添加加密 Channel 验证             | T-ACCESS-002  |
...
| `src/routing/resolve-route.ts`      | Session 隔离             | **中**       |
...
_本威胁模型是一个活动文档。请将安全问题发送至 security@openclaw.ai_

---
mmh3_hash: "1dc7f7614547eacad2d4fce8899d717e"
title: "展示"
...
<Info>
**想要被展示?**在 [Discord 的 #showcase](https://discord.gg/clawd) 分享您的项目,或[在 X 上标记 @openclaw](https://x.com/openclaw)。
</Info>
...
向"Robby"(@openclaw)询问本地酒窖技能。它请求样本 CSV 导出 + 存储位置,然后快速构建/测试技能(示例中有 962 瓶)。
...
桌面应用,用于管理 Agents、Claude、Codex 和 OpenClaw 的技能/命令。
...
Homebrew 安装的帮助程序,用于列出/检查/观察本地 OpenAI Codex 会话(CLI + VS Code)。
...
在一个网关下有 14 个以上的 Agent，Opus 4.5 编排器委托给 Codex 工作者。全面的[技术文章](https://github.com/adam91holt/orchestrated-ai-articles)涵盖梦之队名单、模型选择、沙盒、webhooks、心跳和委托流程。Agent 沙盒的 [Clawdspace](https://github.com/adam91holt/clawdspace)。[博客文章](https://adams-ai-journey.ghost.io/2026-the-year-of-the-orchestrator/)。
</Card>
...
由屋顶相机触发:每当天空看起来漂亮时,要求 OpenClaw 拍摄天空照片 —— 它设计了一个技能并拍摄了照片。
...
OpenClaw 连接到 Jira,然后即时生成一个新技能(在它存在于 ClawHub 上之前)。
</Card>
...
<Card title="Inside-Out-2 记忆" icon="brain">
  **社区** • `memory` `beliefs` `self-model`
  ...
  单独的记忆管理器,将会话文件转换为记忆 → 信念 → 不断演变的自我模型。
</Card>
...
<Steps>
  <Step title="分享它">
    在 [Discord 的 #showcase](https://discord.gg/clawd) 发帖或[发推文 @openclaw](https://x.com/openclaw)
  </Step>
  <Step title="包括详细信息">

...
OpenCode 完成更改 → 打开 PR → OpenClaw 审查差异并在 Telegram 中回复"次要建议"以及明确的合并裁决（包括首先应用的关键修复）。
...
向"Robby"（@openclaw）询问本地酒窖 Skill。它请求样本 CSV 导出 + 存储位置，然后快速构建/测试 Skill（示例中有 962 瓶）。
...
桌面应用，用于管理 Agents、Claude、Codex 和 OpenClaw 的 Skills/命令。
...
Homebrew 安装的帮助程序，用于列出/检查/观察本地 OpenAI Codex Session（CLI + VS Code）。
...
在一个 Gateway 下有 14 个以上的 Agent，Opus 4.5 编排器委托给 Codex 工作者。全面的[技术文章](https://github.com/adam91holt/orchestrated-ai-articles)涵盖梦之队名单、模型选择、沙箱、webhooks、心跳和委托流程。Agent 沙箱的 [Clawdspace](https://github.com/adam91holt/clawdspace)。[博客文章](https://adams-ai-journey.ghost.io/2026-the-year-of-the-orchestrator/)。
</Card>

<Card title="Linear CLI" icon="terminal" href="https://github.com/Finesssee/linear-cli">
  **@NessZerra** • `devtools` `linear` `cli` `issues`

与 Agent 工作流程（Claude Code、OpenClaw）集成的 Linear CLI。从终端管理问题、项目和工作流程。第一个外部 PR 合并！
</Card>
...
计划的提示每天早上通过 OpenClaw 人物生成一个"场景"图像（天气、任务、日期、最喜欢的帖子/引用）。
</Card>
...
在看 Netflix 时通过 Telegram 重建了整个个人网站——Notion → Astro，迁移了 18 篇帖子，DNS 到 Cloudflare。从未打开过笔记本电脑。
</Card>
...
<Card title="Inside-Out-2 记忆" icon="brain">
  **社区** • `memory` `beliefs` `self-model`
  ...
  单独的记忆管理器，将 Session 文件转换为记忆 → 信念 → 不断演变的自我模型。
</Card>
...
<Card title="Clawdia 电话桥" icon="phone" href="https://github.com/alejandroOPI/clawdia-bridge">
  **@alejandroOPI** • `voice` `vapi` `bridge`
  ...
  Vapi 语音助手 ↔ OpenClaw HTTP 桥。与您的 Agent 进行近实时电话呼叫。
</Card>
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  Nix 原生家庭自动化，OpenClaw 作为接口，加上漂亮的 Grafana 仪表板。

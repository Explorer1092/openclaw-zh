...
<Info>
**想被推荐？** 在 [Discord 上的 #showcase](https://discord.gg/clawd) 分享你的项目，或在 [X 上标记 @openclaw](https://x.com/openclaw)。
</Info>
...
<Card title="PR 审查 → Telegram 反馈" icon="code-pull-request" href="https://x.com/i/status/2010878524543131691">
  **@bangnokia** • `review` `github` `telegram`
  ...
  OpenCode 完成更改 → 开启 PR → OpenClaw 审查差异并在 Telegram 中回复“小建议”加上明确的合并裁决（包括首先应用的严重修复）。
...
<Card title="几分钟内的酒窖技能" icon="wine-glass" href="https://x.com/i/status/2010916352454791216">
  **@prades_maxime** • `skills` `local` `csv`
  ...
  要求 “Robby” (@openclaw) 制作一个本地酒窖技能。它请求示例 CSV 导出 + 存储位置，然后快速构建/测试该技能（示例中有 962 瓶）。
...
<Card title="Tesco 购物自动驾驶仪" icon="cart-shopping" href="https://x.com/i/status/2009724862470689131">
  **@marchattonhere** • `automation` `browser` `shopping`
  ...
  每周膳食计划 → 常购物品 → 预订配送时段 → 确认订单。没有 API，只有浏览器控制。
...
<Card title="SNAG 截图转 Markdown" icon="scissors" href="https://github.com/am-will/snag">
  **@am-will** • `devtools` `screenshots` `markdown`
  ...
  热键选取屏幕区域 → Gemini 视觉 → 瞬间将 Markdown 放入剪贴板。
...
<Card title="Agents UI" icon="window-maximize" href="https://releaseflow.net/kitze/agents-ui">
  **@kitze** • `ui` `skills` `sync`
  ...
  用于管理 Agents, Claude, Codex 和 OpenClaw 之间的技能/命令的桌面应用。
...
<Card title="Telegram 语音笔记 (papla.media)" icon="microphone" href="https://papla.media/docs">
  **社区** • `voice` `tts` `telegram`
  ...
  包装 papla.media TTS 并作为 Telegram 语音笔记发送结果（没有烦人的自动播放）。
...
<Card title="Bambu 3D 打印机控制" icon="print" href="https://clawdhub.com/tobiasbischoff/bambu-cli">
  **@tobiasbischoff** • `hardware` `3d-printing` `skill`
  ...
  控制和故障排除 BambuLab 打印机：状态、任务、摄像头、AMS、校准等。
...
<Card title="ParentPay 学校供餐" icon="utensils" href="#">
  **@George5562** • `automation` `browser` `parenting`

  通过 ParentPay 自动预订英国学校供餐。使用鼠标坐标进行可靠的表格单元格点击。
</Card>

<Card title="R2 上传 (给我发文件)" icon="cloud-arrow-up" href="https://clawdhub.com/skills/r2-upload">
  **@julianengel** • `files` `r2` `presigned-urls`

  上传到 Cloudflare R2/S3 并生成安全预签名下载链接。非常适合远程 OpenClaw 实例。
</Card>

<Card title="通过 Telegram 的 iOS 应用" icon="mobile" href="#">
  **@coard** • `ios` `xcode` `testflight`

  构建了一个完整的 iOS 应用程序，带有地图和录音功能，完全通过 Telegram 聊天部署到 TestFlight。
...
<Card title="Oura 戒指健康助手" icon="heart-pulse" href="#">
  **@AS** • `health` `oura` `calendar`
  ...
  个人 AI 健康助手，整合 Oura 戒指数据与日历、预约和健身房时间表。
...
<Card title="Kev 的梦之队 (14+ 智能体)" icon="robot" href="https://github.com/adam91holt/orchestrated-ai-articles">
  **@adam91holt** • `multi-agent` `orchestration` `architecture` `manifesto`
  ...
  14+ 个智能体在一个网关下，由 Opus 4.5 协调者委托给 Codex 工人。全面的 [技术文章](https://github.com/adam91holt/orchestrated-ai-articles) 涵盖了梦之队名单、模型选择、沙盒化、webhook、心跳和委托流程。[Clawdspace](https://github.com/adam91holt/clawdspace) 用于智能体沙盒化。[博客文章](https://adams-ai-journey.ghost.io/2026-the-year-of-the-orchestrator/)。
</Card>

<Card title="Linear CLI" icon="terminal" href="https://github.com/Finesssee/linear-cli">
  **@NessZerra** • `devtools` `linear` `cli` `issues`

  Linear 的 CLI，与智能体工作流 (Claude Code, OpenClaw) 集成。从终端管理问题、项目和工作流。第一个合并的外部 PR！
</Card>

<Card title="Beeper CLI" icon="message" href="https://github.com/blqke/beepcli">
  **@jules** • `messaging` `beeper` `cli` `automation`

  通过 Beeper Desktop 读取、发送和归档消息。使用 Beeper 本地 MCP API，以便智能体可以在一个地方管理你的所有聊天（iMessage, WhatsApp 等）。
</Card>
...
<Card title="Winix 空气净化器控制" icon="wind" href="https://x.com/antonplex/status/2010518442471006253">
  **@antonplex** • `automation` `hardware` `air-quality`
  ...
  Claude Code 发现并确认了净化器控制，然后 OpenClaw 接管以管理房间空气质量。
...
<Card title="美丽的天空相机拍摄" icon="camera" href="https://x.com/signalgaining/status/2010523120604746151">
  **@signalgaining** • `automation` `camera` `skill` `images`
  ...
  由屋顶摄像头触发：要求 OpenClaw 每当天空看起来漂亮时拍一张照片 —— 它设计了一个技能并拍摄了照片。
...
<Card title="视觉早间简报场景" icon="robot" href="https://x.com/buddyhadry/status/2010005331925954739">
  **@buddyhadry** • `automation` `briefing` `images` `telegram`
  ...
  定时提示通过 OpenClaw 人设每天早上生成单个“场景”图像（天气、任务、日期、喜欢的帖子/语录）。
</Card>
...
<Card title="会计摄入" icon="file-invoice-dollar">
  **社区** • `automation` `email` `pdf`
  
  从电子邮件收集 PDF，为税务顾问准备文件。自动驾驶模式下的每月会计。
</Card>

<Card title="沙发土豆开发模式" icon="couch" href="https://davekiss.com">
  **@davekiss** • `telegram` `website` `migration` `astro`

  在看 Netflix 时通过 Telegram 重建了整个个人网站 —— Notion → Astro，迁移了 18 篇文章，DNS 到 Cloudflare。从未打开过笔记本电脑。
</Card>

<Card title="求职智能体" icon="briefcase">
  **@attol8** • `automation` `api` `skill`

  搜索招聘信息，匹配简历关键词，并返回带有链接的相关机会。使用 JSearch API 在 30 分钟内构建。
</Card>

<Card title="Jira 技能构建器" icon="diagram-project" href="https://x.com/jdrhyne/status/2008336434827002232">
  **@jdrhyne** • `automation` `jira` `skill` `devtools`

  OpenClaw 连接到 Jira，然后即时生成一个新技能（在 ClawdHub 上存在之前）。
</Card>
...
<Card title="Slack 自动支持" icon="slack">
  **@henrymascot** • `slack` `automation` `support`
  ...
  监视公司 Slack 频道，提供有用的回复，并将通知转发到 Telegram。自主修复了已部署应用程序中的生产错误，而无需被询问。
</Card>
...
<Card title="Karakeep 语义搜索" icon="magnifying-glass" href="https://github.com/jamesbrooksco/karakeep-semantic-search">
  **@jamesbrooksco** • `search` `vector` `bookmarks`
  ...
  使用 Qdrant + OpenAI/Ollama 嵌入向 Karakeep 书签添加向量搜索。
</Card>

<Card title="头脑特工队-2 记忆" icon="brain">
  **社区** • `memory` `beliefs` `self-model`
  
  独立的记忆管理器，将会话文件转化为记忆 → 信念 → 进化的自我模型。
</Card>
...
<Card title="Clawdia 电话桥接" icon="phone" href="https://github.com/alejandroOPI/clawdia-bridge">
  **@alejandroOPI** • `voice` `vapi` `bridge`
  ...
  Vapi 语音助手 ↔ OpenClaw HTTP 桥接。与你的智能体进行近实时通话。
</Card>
...
<Card title="Nix 打包" icon="snowflake" href="https://github.com/openclaw/nix-openclaw">
  **@openclaw** • `nix` `packaging` `deployment`
  ...
  用于可重复部署的电池包含的 nixified OpenClaw 配置。
</Card>
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  以 OpenClaw 为接口的 Nix 原生家庭自动化，外加漂亮的 Grafana 仪表盘。
...
<Card title="StarSwap 市场" icon="star" href="https://star-swap.com/">
  **社区** • `marketplace` `astronomy` `webapp`
  ...
  完整的天文装备市场。使用/围绕 OpenClaw 生态系统构建。
</Card>
...
<Steps>
  <Step title="分享它">
    在 [Discord 上的 #showcase](https://discord.gg/clawd) 发布或 [推特 @openclaw](https://x.com/openclaw)
  </Step>
  <Step title="包含详情">
    告诉我们它是做什么的，链接到仓库/演示，如果有截图请分享
  </Step>
  <Step title="被推荐">
    我们会将杰出的项目添加到此页面
  </Step>

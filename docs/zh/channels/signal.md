...
sidebarTitle: "Signal"
mmh3_hash: "77ed5c3439dfcf31d96913dcefa1028f"
summary: "通过 signal-cli (JSON-RPC + SSE) 提供 Signal 支持、设置和号码模型"
read_when: ["Setting up Signal support","Debugging Signal send/receive"]
---
# Signal (signal-cli)
...
## 它是什么
- 通过 `signal-cli` 提供 Signal 频道（非嵌入式 libsignal）。
- 确定性路由：回复始终返回到 Signal。
- 私聊共享 agent 的主会话；群组是隔离的（`agent:<agentId>:signal:group:<groupId>`）。
...
多账号支持：使用 `channels.signal.accounts` 配置每个账号，可选 `name`。参见 [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) 了解共享模式。
...
- 未知发送者会收到配对码；在批准之前消息将被忽略（验证码 1 小时后过期）。
- 批准方式：
  - `openclaw pairing list signal`
  - `openclaw pairing approve signal <CODE>`
- 配对是 Signal 私聊的默认 token 交换方式。详情：[配对](/start/pairing)
...
- `channels.signal.allowFrom`：私聊允许列表（E.164 或 `uuid:<id>`）。`open` 需要 `"*"`。Signal 没有用户名；使用电话/UUID id。
...
- `channels.signal.dmHistoryLimit`：用户回合的私聊历史限制。每用户覆盖：`channels.signal.dms["<phone_or_uuid>"].historyLimit`。

...
DM allowlist（空 = 不允许 DM，使用 `ownerShip` 进行审批流程）：
...
- **群组**：列表、创建、管理成员
- **私信**：发送消息、对消息做出 Reaction
- **Reaction**：添加/移除帖子和私信的表情 Reaction
...
- `channels.tlon.authorization.channelRules`：每 Channel 认证规则。

...
Nostr 是一个去中心化的社交网络协议。此频道使 OpenClaw 能够通过 NIP-04 接收和回复加密的私信 (DM)。

## 按需安装

### 引导向导(推荐)

- 引导向导 (`openclaw onboard`) 和 `openclaw channels add` 会列出可选的频道插件。
- 选择 Nostr 会提示你按需安装插件。

默认安装方式:

- **开发版频道 + git 检出可用:** 使用本地插件路径。
- **稳定版/测试版:** 从 npm 下载。
...
个人资料数据作为 NIP-01 `kind:0` 事件发布。你可以从控制界面(频道 -> Nostr -> 个人资料)管理它,或直接在配置中设置。

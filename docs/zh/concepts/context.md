...
Context _不同于_"内存"：内存可以存储在磁盘上并稍后重新加载；context 是 model 当前窗口内的内容。
...
Skill 指令_默认不包含_。期望 model **仅在需要时** `read` skill 的 `SKILL.md`。
...
- **Pruning** 从运行的_内存中_ prompt 中删除旧的 tool results，但不重写 transcript。
...
- `System prompt (estimate)` = 当没有运行报告时即时计算。

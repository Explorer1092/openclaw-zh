---
mmh3_hash: "df6cd27cf8ff31637374ddb3ac7f9d9a"
...
在一个运行的 Gateway 中运行多个 _隔离的_ agents——每个都有自己的 workspace、state directory（`agentDir`）和 session 历史记录——加上多个 channel accounts（例如两个 WhatsApps）。入站消息通过 bindings 路由到正确的 agent。

**Agent** 在这里是完整的每 persona 范围：workspace 文件、auth profiles、model registry 和 session store。`agentDir` 是保存每个 agent 配置的磁盘 state directory，位于 `~/.openclaw/agents/<agentId>/`。**Binding** 将一个 channel account（例如 Slack workspace 或 WhatsApp 号码）映射到其中一个 agents。
...
<Note>
`sessions_history` 在这里也是更安全的跨 session 召回路径：它返回有边界的、经过清理的视图，而非原始 transcript 转储。Assistant 召回在编辑/截断之前会剥离 thinking 标签、`<relevant-memories>` 脚手架、纯文本工具调用 XML payloads（包括 `<tool_call>...</tool_call>`、`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、`<function_calls>...</function_calls>` 以及截断的工具调用块）、降级的工具调用脚手架、泄露的 ASCII/全角 model 控制令牌以及格式错误的 MiniMax 工具调用 XML，然后再编辑/截断。
</Note>

<Warning>
Main agent 凭据**不**自动共享。永远不要在 agents 之间重用 `agentDir`（会导致 auth/session 冲突）。如果你想共享 creds，将 `auth-profiles.json` 复制到另一个 agent 的 `agentDir`。
</Warning>
...
<Steps>
  ...
  <Step title="创建 channel accounts">
    在你偏好的 channels 上为每个 agent 创建一个 account：
...
额外集合路径可以在 agents 之间共享，但当路径在 agent workspace 之外时，集合名称保持显式。workspace 内的路径保持 agent 范围，因此每个 agent 保留自己的记录搜索集合。
...
<Note>
直接聊天折叠到 agent 的 **main session key**，因此真正的隔离需要 **每个人一个 agent**。
</Note>
...
<Steps>
  ...
  <Step title="默认 agent">
    后备到 `agents.list[].default`，否则第一个列表条目，默认：`main`。
  </Step>
</Steps>

<AccordionGroup>
  <Accordion title="打平规则和 AND 语义">
    - 如果同一层级中有多个 bindings 匹配，则配置顺序中的第一个获胜。
    - 如果一个 binding 设置了多个匹配字段（例如 `peer` + `guildId`），则所有指定字段都是必需的（`AND` 语义）。
  </Accordion>
  <Accordion title="Account-scope 细节">
...
<Tabs>
  <Tab title="WhatsApp 日常聊天 + Telegram 深度工作">
  ...
  <Tab title="同一 channel，一个 peer 到 Opus">
  ...
  <Tab title="绑定到 WhatsApp group 的家庭 agent">
...
每个 agent 可以有自己的 sandbox 和 tool 限制：
...
<Note>
`tools.elevated` 是 **全局的** 并且基于发送者；它不能按 agent 配置。如果你需要每个 agent 的边界，使用 `agents.list[].tools` 拒绝 `exec`。对于 group 定位，使用 `agents.list[].groupChat.mentionPatterns`，以便 @mentions 清晰地映射到预期的 agent。
</Note>

参见 [Multi-agent sandbox and tools](/tools/multi-agent-sandbox-tools) 了解详细示例。

## Related

- [ACP agents](/tools/acp-agents) — 运行外部 coding harnesses
- [Channel routing](/channels/channel-routing) — 消息如何路由到 agents
- [Presence](/concepts/presence) — agent presence 和可用性

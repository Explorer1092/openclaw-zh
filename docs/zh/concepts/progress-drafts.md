---
mmh3_hash: "c817eac3191b3f3896f70b9561a97915"
...
| 部分       | 用途                                                                                |
| ---------- | ----------------------------------------------------------------------------------- |
| 标签       | 简短的启动/状态行，如 `Thinking...` 或 `Shelling...`。                              |
...
默认标签为 `auto`，从 OpenClaw 内置的单词加省略号标签池中选取：
...
OpenClaw 默认截断长进度行，避免重复的草稿编辑因换行不同而产生差异。前缀保持可读，长详情（如路径或原始命令）以省略号缩短。

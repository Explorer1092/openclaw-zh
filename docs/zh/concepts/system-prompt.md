...
- 当您需要检查正在运行的命令时，使用 `process` 查看日志、状态、输入或干预
...
在具有原生批准卡片/按钮的 Channel 上，运行时 prompt 现在告诉 Agent 首先依赖该原生批准 UI。只有当 tool result 表示聊天批准不可用或手动批准是唯一路径时，才应包含手动 `/approve` 命令。

## Prompt 模式

OpenClaw 可以为子 Agent 渲染较小的 system prompts。运行时为每次运行设置 `promptMode`（不是面向用户的配置）：

- `full`（默认）：包括上面的所有部分。
- `minimal`：用于子 Agent；省略 **Skills**、**Memory Recall**、**OpenClaw Self-Update**、**Model Aliases**、**User Identity**、**Reply Tags**、**Messaging**、**Silent Replies** 和 **Heartbeats**。Tooling、**Safety**、Workspace、Sandbox、Current Date & Time（在已知时）、Runtime 和注入的 context 保持可用。
- `none`：仅返回基础身份行。
...
子 Agent Session 仅注入 `AGENTS.md` 和 `TOOLS.md`（其他 bootstrap 文件被过滤掉以保持子 Agent context 小）。
...
这种分离使 Skills 大小与运行时读取/注入大小分开，例如 `memory_get`、实时工具结果和 compaction 后的 AGENTS.md 刷新。

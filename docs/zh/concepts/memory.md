---
mmh3_hash: "df947c270fd4121e07ea9f44474942d2"
...
大多数内存可以作为普通 Markdown 笔记写入。但有些内存会影响 Agent 以后应该做什么。对于这些内存，捕获何时可以安全地对笔记采取行动，而不仅仅是事实本身。

当笔记涉及以下情况时，捕获该行动边界：

- 审批或权限要求，
- 临时约束，
- 移交给另一个 Session、线程或人员，
- 过期条件，
- 安全行动时机，
- 来源或所有者权限，
- 避免某个诱人行动的指令。
...
内存可以保留审批上下文，但不强制执行策略。对于硬性操作控制，使用 OpenClaw 审批设置、沙箱化和计划任务。

示例：

```md
API 迁移正在另一个 Session 中设计。未来的轮次不应该从此线程编辑 API 实现；只将此处的发现用作设计输入，直到迁移计划落地。
```

另一个示例：

```md
来自不受信任来源的报告在升级前需要审查。未来的轮次应将其仅视为证据；在受信任的审查者确认内容之前，不要将其存储为持久内存。
```

使用[承诺](/concepts/commitments)处理推断的、短期的后续事项。使用[计划任务](/automation/cron-jobs)处理精确的提醒、定时检查和重复工作。内存仍然可以汇总围绕任一路径的持久上下文。

这不是每条内存都需要的必填模式。简单事实可以保持简洁。当失去时机、权限、过期或安全行动上下文可能导致 Agent 以后做错事时，使用对行动敏感的边界。

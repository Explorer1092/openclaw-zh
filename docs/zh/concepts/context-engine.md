---
mmh3_hash: "3816c2db74b70c8d0c712ca544c336a5"
...
**context engine** 控制 OpenClaw 如何为每次运行构建 model context：包含哪些消息、如何总结较旧的历史记录，以及如何跨子 Agent 边界管理 context。

OpenClaw 内置一个 `legacy` engine，并默认使用它——大多数用户不需要更改此设置。仅当你想要不同的组装、compaction 或跨 Session 召回行为时，才需要安装和选择 plugin engine。

## 快速开始

<Steps>
  <Step title="检查哪个 engine 处于活动状态">
    ```bash
    openclaw doctor
    # 或直接检查配置:
  ...
  <Step title="安装 plugin engine">
    Context engine plugin 的安装方式与其他 OpenClaw plugin 相同。
...
<AccordionGroup>
  <Accordion title="1. Ingest">
    当新消息添加到 Session 时调用。Engine 可以在其自己的数据存储中存储或索引消息。
  </Accordion>
  <Accordion title="2. Assemble">
    在每次 model 运行之前调用。Engine 返回一组有序的消息（以及可选的 `systemPromptAddition`），以符合 token 预算。
  </Accordion>
  <Accordion title="3. Compact">
    当 context window 已满时，或当用户运行 `/compact` 时调用。Engine 总结较旧的历史记录以释放空间。
  </Accordion>
  <Accordion title="4. After turn">
    在运行完成后调用。Engine 可以持久化状态、触发后台 compaction 或更新索引。
  </Accordion>
</AccordionGroup>

对于捆绑的非 ACP Codex harness，OpenClaw 通过将组装的 context 投影到 Codex 开发者指令和当前回合 prompt 中来应用相同的生命周期。Codex 仍然拥有其原生线程历史记录和原生 compactor。
...
<ParamField path="prepareSubagentSpawn" type="method">
  在子运行开始之前准备共享 context 状态。Hook 接收父/子 Session 键、`contextMode`（`isolated` 或 `fork`）、可用的 transcript id/文件和可选的 TTL。如果它返回回滚句柄，OpenClaw 会在准备成功后 spawn 失败时调用它。
</ParamField>
<ParamField path="onSubagentEnded" type="method">
  当子 Agent Session 完成或被清理时进行清理。
</ParamField>

### System prompt addition

`assemble` 方法可以返回一个 `systemPromptAddition` 字符串。OpenClaw 将其插入到运行的 system prompt 开头。这让 engine 可以注入动态召回指导、检索指令或上下文感知提示，而无需静态 workspace 文件。
...
<ParamField path="estimatedTokens" type="number" required>
  Engine 对组装 context 中总 tokens 的估计。OpenClaw 将此用于 compaction 阈值决策和诊断报告。
</ParamField>
<ParamField path="systemPromptAddition" type="string">
  插入到 system prompt 开头。
</ParamField>

`compact` 返回一个 `CompactResult`。当 compaction 轮换活跃转录时，`result.sessionId` 和 `result.sessionFile` 标识下一次重试或回合必须使用的后继 Session。
...
| `prepareSubagentSpawn(params)` | 方法   | 在子 Session 开始之前设置共享状态。                                                                             |
...
<AccordionGroup>
  <Accordion title="ownsCompaction: true">
    Engine 拥有 compaction 行为。OpenClaw 为该运行禁用 Pi 的内置自动 compaction，engine 的 `compact()` 实现负责 `/compact`、溢出恢复 compaction 以及它想在 `afterTurn()` 中做的任何主动 compaction。OpenClaw 仍可能运行预提示溢出保障；当它预测完整转录会溢出时，恢复路径会在提交另一个 prompt 之前调用活动 engine 的 `compact()`。
  </Accordion>
  <Accordion title="ownsCompaction: false 或未设置">
    Pi 的内置自动 compaction 在 prompt 执行期间仍可能运行，但活动 engine 的 `compact()` 方法仍用于 `/compact` 和溢出恢复。
  </Accordion>
</AccordionGroup>
...
<Tabs>
  <Tab title="Owning mode">
    实现你自己的 compaction 算法并设置 `ownsCompaction: true`。
  </Tab>
  <Tab title="Delegating mode">
    设置 `ownsCompaction: false` 并让 `compact()` 从 `openclaw/plugin-sdk/core` 调用 `delegateCompactionToRuntime(...)` 以使用 OpenClaw 的内置 compaction 行为。
  </Tab>
</Tabs>
...
<Note>
**Plugin 卸载：** 当你卸载当前选为 `plugins.slots.contextEngine` 的 plugin 时，OpenClaw 会将 slot 重置回默认值（`legacy`）。相同的重置行为适用于 `plugins.slots.memory`。不需要手动编辑配置。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Session pruning">
    在内存中修剪旧工具结果仍然运行，无论哪个 context engine 处于活动状态。
  </Accordion>
</AccordionGroup>
...
- [Plugin manifest](/plugins/manifest) — plugin manifest 字段
- [Plugins](/tools/plugin) — plugin 概述
